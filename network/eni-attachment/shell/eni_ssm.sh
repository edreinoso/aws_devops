...
aws ec2 attach-network-interface --device-index 1 --instance-id i-0735ec11daade7d19 --network-interface-id eni-048056b45825f5125
...
aws ssm update-document --name "eni-attachment" --content file://ssm.json --document-version '$LATEST'; aws ssm update-document-default-version --name "eni-attachment" --document-version "25"; aws lambda invoke --function-name eni-attachment --payload fileb://payload.json response.json
