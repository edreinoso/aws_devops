...
ebs_key = '45f02b69-219a-4fad-b6e3-44056c46ca1c'
global_key = 'cabbdf47-58d5-48f0-b853-33d587c3cfde'

# These keys are from ohio regionand they represents aws_ebs and global_kms
aws_ebs = 'arn:aws:kms:us-east-2:698236466819:key/590f9f50-407e-42b7-b575-841c9fe9986c'
global_kms = 'arn:aws:kms:us-east-2:698236466819:key/ddb2221a-6c5a-4b6c-9bb6-82695eea58ae'
        ...
        if i > 5:
            ...
            for tags in snapshot['Tags']:
                ...
                if (tags['Key'] == 'Name'):
                    ...
                    if(snapshot['KmsKeyId'] == aws_ebs):
                        print('###encryption with aws_ebs')
                        new_snapshot_id = copy_snapshot(
                            snapshot_id, snapshot_name, key_name_tag, snapshot_tag_name, global_key)
                    elif(snapshot['KmsKeyId'] == global_kms):
                        print('###encryption with global_kms')
                        new_snapshot_id = copy_snapshot(
                            snapshot_id, snapshot_name, key_name_tag, snapshot_tag_name, global_key)
...
def copy_snapshot(snapshot_id, snapshot_name, key_name_tag, snapshot_tag_name, kms_key):
